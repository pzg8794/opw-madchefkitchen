								...
								if($_FILES)
								{
									...
									if($_FILES['file']['name'] != "") 
									{
											...
											for ($c=0; $c < $num; $c++)
											{
												...
												else
												{
													...
													if(empty($data[$c]) && $c==1)
														...
														$recipe[] = nl2br(mysqli_real_escape_string($dbcon, trim($data[$c])));
										...
										{ 
											...
											AND Rsummary ='$rs' AND Ringredients ='$recipe[5]' AND Rinstructions = '$recipe[6]' AND Rprep_time='$rpt' AND Rcoock_time = '$rct'
